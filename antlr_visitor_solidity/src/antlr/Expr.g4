...
versionConstraint
  : versionOperator? VersionLiteral            #VersionCheck
  ;
...
modifierList
  : ( modifierInvocation | stateMutability | ExternalKeyword| PublicKeyword | InternalKeyword | PrivateKeyword )* #checkModifier
  ;
eventDefinition
...
expression :  
   ...
   |'new' typeName   #newKeyword
   | expression '[' expression ']' #exprSqBr
   | expression '(' functionCallArguments ')'  #exprFnc
   | expression '.' identifier              #ExprCheck     
   | '(' expression ')'  #exprRoBr
   | ('++' | '--') expression  #exprInc
   | ('+' | '-') expression  #exprPlus
   | ('after' | 'delete') expression #exprdel
   | '!' expression #exprNot
   | '~' expression  #exprTilde
   | expression '**' expression #exprMul
   | expression ('*' | '/' | '%') expression  #exprcalc
   | expression ('+' | '-') expression  #exprPlusMinus
   | expression ('<<' | '>>') expression #exprGreater
   | expression '&' expression  #exprAnd
   | expression '^' expression  #exprXor
   | expression '|' expression  #exorOr
   | expression ('<' | '>' | '<=' | '>=') expression  #exprCompare
   | expression ('==' | '!=') expression  #exprCmp
   | expression '&&' expression  #exprLogicalAnd
   | expression '||' expression  #exprLogicalOr
   | expression '?' expression ':' expression  #exprTernary
   | expression ('=' | '|=' | '^=' | '&=' | '<<=' | '>>=' | '+=' | '-=' | '*=' | '/=' | '%=') expression #exprAll
   | primaryExpression #exprPrime
  ;
